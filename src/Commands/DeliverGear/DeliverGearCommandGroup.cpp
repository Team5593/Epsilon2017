...
DeliverGearCommandGroup::DeliverGearCommandGroup(double rotationAngle, double firstForwardDistance) {
	...
	_driveSetDistanceCommand = new DriveSetDistanceCommand();
	_driveSetDistanceCommand->Initialize(firstForwardDistance);
	...
	AddSequential(_driveSetDistanceCommand, DRIVE_FORWARDS_COMMAND_EXPIRY_SECONDS);
